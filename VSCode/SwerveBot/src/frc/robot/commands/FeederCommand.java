...
public class FeederCommand extends Command {
  ...
  private final static double k_reverseTime = 0.2;
  private final static double k_reversePower = 0.5;
  private boolean m_stalled = false;
  private final Timer m_timer = new Timer();
  private final DigitalCounter m_ballCounter;
   ...
   */
  public FeederCommand(FeederSubsystem subsystem, DigitalCounter ballCounter, boolean reset, int count) {
  ...
  public boolean isFinished() {
    ...
    if (m_count > 0) {
      Logger.log("FeederCommand", 1, String.format("Ball count = %d", m_ballCounter.get()));

      return (m_ballCounter.get() >= m_count);
    }
    return false;
  }
}
